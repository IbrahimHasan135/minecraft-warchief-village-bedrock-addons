...
export function registerPhase03ConscriptionCommandLoop(): void {
...
function conscriptVillager(player: Player, villager: Entity, sourceSlotIndex: number): void {
...
function updateBannerCommands(): void {
...
function commandOwnedSoldiersToFollow(player: Player): void {
...
function getNearbyOwnedSoldiers(player: Player): Entity[] {
  try {
...
function isHoldingVanillaBanner(player: Player): boolean {
...
function slotHasItem(player: Player, expectedTypeId: string, slotIndex: number): boolean {
  const inventory = player.getComponent(EntityComponentTypes.Inventory) as EntityInventoryComponent | undefined;
  const item = inventory?.container?.getItem(slotIndex);
  return item?.typeId === expectedTypeId;
}

function consumeItemFromSlot(player: Player, expectedTypeId: string, slotIndex: number): boolean {
  const inventory = player.getComponent(EntityComponentTypes.Inventory) as EntityInventoryComponent | undefined;
...
function safeRemove(entity: Entity): void {
  try {
    entity.remove();
  } catch (error) {
